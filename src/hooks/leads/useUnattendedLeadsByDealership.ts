...
export const fetchUnattendedLeadsByDealership = async (selectedDealershipId?: string) => {
  try {
    let query = supabase
    ...
    if (selectedDealershipId) {
      query = query.eq("dealership_id", selectedDealershipId); // Filter by dealership
    }

    const { data: unattendedLeads, error: unattendedError } = await query;
